...
(function($) {
        ...
        path: { // 路径转换
            ...
            props: {
                ...
                text: {
                    ...
                    value: '',
                    editor: function() {
                        return new myflow.editors.selectEditor(new Array({'value':'1','name':'success'},{'value':'2','name':'fail'},{'value':'3','name':'*'},{'value':'4','name':'case1'},{'value':'5','name':'case2'},{'value':'6','name':'case3'}));
                    }
                }
            }
        },
    ...
    myflow.rect = function(o, r, id) {
            ...
            function() {
                ...
                switch (mod) {
                    ...
                    case 'path':
                        ...
                        //开始节点不能有指向decision和join的箭头
                        if (legalpath1 && legalpath2 && states[pre.getId()].type == "start" ) {
                            var layerMsg = "";
                            if(states[_id].type == "decision"){
                                layerMsg = '【开始】节点不能有指向【判定】的流程!';
                            }
                            if(states[_id].type == "join"){
                                layerMsg = '【开始】节点不能有指向【聚合】的流程!';
                            }
                            if(states[_id].type == "end"){
                                layerMsg = '【开始】节点不能有指向【结束】节点的流程!';
                            }
                            if(layerMsg != ""){
                                layer.open({
                                    type: 1,
                        ...
                        //fork不能指向。。。
                        if(legalpath1 && legalpath2 && states[pre.getId()].type == "fork"){
                            var layerMsg = "";
                            if(states[_id].type == "end"){
                                layerMsg = '【并行】节点不能有指向【结束】节点的流程!';
                            }
                            if(states[_id].type == "decision"){
                                layerMsg = '【并行】节点不能有指向【判定】节点的流程!';
                            }
                            if(states[_id].type == "join"){
                                layerMsg = '【并行】节点不能有指向【聚合】节点的流程!';
                            }
                            if(states[_id].type == "fork"){
                                layerMsg = '【并行】节点不能有指向【并行】节点的流程!';
                            }
                            if(layerMsg != ""){
                                layer.open({
                                    type: 1,
                        ...
                        //decision不能指向。。。
                        if(legalpath1 && legalpath2 && states[pre.getId()].type == "decision"){
                            var layerMsg = "";
                            if(states[_id].type == "decision"){
                                layerMsg = '【判定】节点不能有指向【判定】节点的流程!';
                            }
                            if(states[_id].type == "join"){
                                layerMsg = '【判定】节点不能有指向【聚合】节点的流程!';
                            }
                            if(layerMsg != ""){
                                layer.open({
                                    type: 1,
                        ...
                        //并行只能有一个进入箭头
                        if (legalpath1 && legalpath2 && (states[_id].type == "fork" || states[_id].type == "decision")) {
                            for (var k in paths) {
                                if (paths[k].to == _id) {
                                    layer.open({
                                        type: 1,
                        ...
                        //task只能有一个出去的箭头
                        if (legalpath1 && legalpath2 && states[pre.getId()].type == "task") {
                            for (var k in paths) {
                                if (paths[k].from == pre.getId()) {
                                    layer.open({
                                        type: 1,
                        ...
                        //task只能有一个进入箭头
                        // if (legalpath1 && legalpath2 && states[_id].type == "task") {
                        //     for (var k in paths) {
                        //         if (paths[k].to == _id) {
                        //             layer.open({
                        //                 type: 1,
        ...
        function resize() {
            ...
            switch (_o.showType) {
                ...
                case 'image&text':
                    _rect.show();
                    if(_o.type == "decision"){
                        $(_rect.node).css("-webkit-clip-path","polygon(50% 0, 100% 50%,50% 100%,0 50%)");
                    }
                    _name.attr({
        ...
        this.toJson = function(rect) {
            ...
            for (var k in _o.props) {
                //rect存在的任务节点  IP字段execIp的值从myflow中获取 更新数据
                if(rect && k == "execIp" && myflow.config.restore.states[rect] != null){
                    _o.props[k].value = myflow.config.restore.states[rect].props.execIp.value;
                }
                data += '"' + k + '":{"value":"' + _o.props[k].value + '"},';
                console.log(_o.props[k].value);
            }
    ...
    myflow.path = function(o, r, from, to) {
        ...
        if (fromObj.type == 'decision') {
            //if (fromObj.type == 'fork' || fromObj.type == 'decision') {
        ...
        var textchangeHandler = function(e, v, src) {
            ...
            var hideTxt = true;
            for (var s in states) {
                if (paths[_id].from == s && (states[s].type == 'decision')) {
                    //if (paths[_id].from == s && (states[s].type == 'fork' || states[s].type == 'decision')) {
                    hideTxt = false;
                }
            }
            var fromObj = JSON.parse(from.toJson());
        ...
        var showpropsHandler = function(e, O, src) {
            ...
            if (_src.getId().indexOf('path') >= 0) {
                ...
                for (var rect in states) {
                    if (rect == from && (states[rect].type != 'decision')) {
                        _pdiv.hide();
                        return;
                    }
                }
            ...
            for (var k in props) {
                //TODO
                if(props[k].label != "0" && props[k].label != "btn"){
                    _tb.append('<tr><th>' + props[k].label + '</th><td><div id="p' + k + '" class="editor"></div></td></tr>');
                }else if(props[k].label == "btn"){
                    _tb.append('<tr><td colspan="2"><div id="p' + k + '" class="editor"></div></td></tr>');
                }else if(props[k].label == "0"){

                }

                if (props[k].editor) props[k].editor().init(O, k, 'p' + k, src, _r,states);
    ...
    myflow.init = function(c, o) {
        ...
        //只能有一个开始and结束节点
        var hasStartOrEndFun = function(statesVal){
            var states = myflow.config.restore.states;
            for (var k in states) {
                if(states[k].type == statesVal){
                    var text = statesVal == "start" ? "开始" : "结束";
                    var top = $(window).height() / 2 - 155 / 2;
                    var topVal = top + 'px';
                    layer.open({
                      type: 1,
                      shade: 0.3,
                      title: false, //不显示标题
                      content: '<p>一个任务流程只能有一个【'+text+'】节点！</p>',
                      cancel: function(){
                        layer.close();
                      }
                    });
        ...
        $(document).keydown(function(e) {
            ...
            if (ctrlDown && e.keyCode == cKey){
                ...
                if(actId != '' && $('#' + actId).parents('#myflow_props').length > 0 ||$('#' + actId).parents("#nodePropertyCon").length > 0 || $('#' + actId).parents("#startPropertyCon").length > 0) return;
            ...
            if (ctrlDown && e.keyCode == vKey){
                ...
                if (vNode.type == "start" || vNode.type == "end") {
                    //判断是否已经存在开始了
                    hasStartOrEnd = hasStartOrEndFun(vNode.type);
                }
                if(!hasStartOrEnd) {
        ...
        if (myflow.config.editable) {
            ...
            //从工具栏拖拽出节点
            var taskNum = 1,forkNum = 1,decisionNum = 1, joinNum = 1;
            $(c).droppable({
                accept: '.state',
                drop: function(event, ui) {
                    ...
                    if (id == "开始" || id == "结束") {
                        //判断是否已经存在开始或者结束
                        var statesVal = id == "开始" ? "start" : "end";
                        hasStartOrEnd = hasStartOrEndFun(statesVal);
                        ...
                        //TODO  验证拖拽重名
                        var myflowStates = myflow.config.restore.states;
                        for(var x in myflowStates){
                            if(id == myflowStates[x].text.text){
                                layer.open({
                                    type: 1,
                                    shade: 0.3,
                                    title: false, //不显示标题
                                    content: '<p>流程图中已存在命名为【'+id+'】的'+myflowStates[x].type+'节点!</p>',
                                    cancel: function(){
                                        layer.close();
                                    }
                                });
                    ...
                    if(!hasStartOrEnd) {
                        //TODO
                        var cx, cy ;
                        if(id.indexOf("并行") >= 0 || id.indexOf("聚合") >= 0){
                            cx = ui.helper.offset().left - 110;
                            cy = ui.helper.offset().top - 44;
                        }else{
                            cx = ui.helper.offset().left - 130;
                            cy = ui.helper.offset().top - 40;
                        }
                        if (cx < 0) cx = 0;
                        if (cy < 0) cy = 0;
            ...
            $('#myflow_save').click(function() {
                ...
                for (var k in _states) {
                    if (_states[k]) {
                        data += '"' + _states[k].getId() + '":' + _states[k].toJson(k) + ',';
                        //任务节点  更新execIp数据
                        if(_states[k].text().indexOf("任务") != -1){
                            JSON.parse(_states[k].toJson()).props.execIp.value = myflow.config.restore.states[k].props.execIp.value;
                        }
                    }
                }
                if (data.substring(data.length - 1, data.length) == ',')
